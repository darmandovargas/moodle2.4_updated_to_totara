...
class block_course_list extends block_list {
    ...
    function get_content() {
        ...
        if (empty($CFG->disablemycourses) and isloggedin() and !isguestuser() and
          !(has_capability('moodle/course:update', context_system::instance()) and $adminseesall) and false) {    // Just print My Courses
        ...
        if ($categories) {   //Check we have categories
            ...
            /// If we can update any course of the view all isn't hidden, show the view all courses link
                if (has_capability('moodle/course:update', context_system::instance()) || empty($CFG->block_course_list_hideallcourseslink)) {
                    $this->content->footer .= "<a href=\"$CFG->wwwroot/course/index.php\">".get_string('fulllistofcourses').'</a> ...';
		      $this->content->footer .= '<form method="get" action="'.$CFG->wwwroot.'/course/search.php" style="margin-top: 1em; text-align: left;"><fieldset><input type="text" value="" name="search" size="20" id="coursesearchbox"><input type="submit" value="Search"></fieldset></form>';
