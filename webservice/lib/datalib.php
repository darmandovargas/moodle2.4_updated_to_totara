...
function get_courses_search($searchterms, $sort='fullname ASC', $page=0, $recordsperpage=50, &$totalcount) {
    ...
    if ($DB->get_dbfamily() == 'oracle') {
        $concat = $DB->sql_concat('c.summary', "' '", 'c.fullname', "' '", 'c.idnumber', "' '", 'c.shortname', 's.name');
		$concatScorm = $DB->sql_concat('s.name');
    } else {
        $concat = $DB->sql_concat("COALESCE(c.summary, '". $DB->sql_empty() ."')", "' '", 'c.fullname', "' '", 'c.idnumber', "' '", 'c.shortname', 's.name');
		$concatScorm = $DB->sql_concat('s.name');
    }
    ...
    /// simpler LIKE (or NOT LIKE) queries
        ...
        if (substr($searchterm,0,1) == '+') {
			...
			$searchcondScorm[] = "$concatScorm $REGEXP :ss$i";
			...
			$searchcondScorm[] = "$concatScorm $NOTREGEXP :ss$i";
            $params['ss'.$i] = "(^|[^a-zA-Z0-9])$searchterm([^a-zA-Z0-9]|$)";

        } else {
            $searchcond[] = $DB->sql_like($concat,":ss$i", false, true, $NOT);
			$searchcondScorm[] = $DB->sql_like($concatScorm,":ss$i", false, true, $NOT);
	...
	$searchcondScorm = implode(" AND ", $searchcondScorm);
	...
	$sqlScorm = "SELECT s.id AS scorm_id, s.name AS scorm_name, c.* $ccselect
              FROM {course} c              
           $ccjoin
           INNER JOIN mdl_scorm s ON (s.course=c.id)
             WHERE $searchcondScorm AND c.id <> ".SITEID."
          ORDER BY $sort";	
    ...
    $rsScorm = $DB->get_recordset_sql($sqlScorm, $params);
    
    foreach($rsScorm as $course) {
        context_instance_preload($course);
        $coursecontext = context_course::instance($course->id);
        if ($course->visible || has_capability('moodle/course:viewhiddencourses', $coursecontext)) {
            // Don't exit this loop till the end
...
/**
 * A list of scorms that match a search
 *
 * @author Diego Vargas
 * @global object
 * @global object
 ...
 * @return object {@link $COURSE} records
 */
function get_scorms_search($searchterms, $sort='fullname ASC', $page=0, $recordsperpage=50, &$totalcount) {
    global $CFG, $DB;
	...
	$sqlScorm = "SELECT cm.id AS course_module_id, s.id AS scorm_id, s.name AS scorm_name, c.* $ccselect
              FROM {course} c             
           $ccjoin
           INNER JOIN mdl_scorm s ON (s.course=c.id)
           INNER JOIN mdl_course_modules cm ON (cm.instance=s.id AND cm.course=c.id)
             WHERE $searchcondScorm AND c.id <> ".SITEID."
          ORDER BY $sort";    
    ...
    foreach($rsScorm as $course) {
        ...
        if ($course->visible || has_capability('moodle/course:viewhiddencourses', $coursecontext)) {
            ...
            // to update $totalcount
            if ($c >= $limitfrom && $c < $limitto) {
            	//gets the courses that match with scorms
            	$courses[$course->id][] = $course;                
            }
            $c++;
        }
