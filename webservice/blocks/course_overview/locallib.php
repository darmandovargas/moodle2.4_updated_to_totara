...
function block_course_overview_get_max_user_courses() {
	...
	*/	
	...
	//Custom Block
	//In case the user have a preference saved, then use it, 
	//other way use the default limit (0 which means all)		
	if(get_user_preferences('course_overview_number_of_courses')>0){
		$limit = get_user_preferences('course_overview_number_of_courses');
	}
...
function block_course_overview_get_sorted_courses() {
    ...
    $courses = enrol_get_my_courses('id, shortname, fullname, modinfo, sectioncache, summary','fullname ASC');
    $site = get_site();
