...
function scorm_course_format_display($user, $course) {
    ...
    if ($scorm->lastattemptlock == 0 || $result->attemptleft > 0) {
        ...
        ?>
              ...
              <input type="submit" class="enter_scorm" value="<?php print_string('enter', 'scorm') ?>" />
              </form>
          </div>
        <?php
    }
        ...
        $headertext .= '</td></tr><tr><td'.$colspan.'>'.'<br /><br /><b style="font-family:ProximaNovaSoft-Regular; font-size:15px;">'.get_string('summary').'</b>'.':<br />'.format_module_intro('scorm', $scorm, $scorm->coursemodule).'</td></tr></table>';
	?> 
	<?php
...
function scorm_view_display ($user, $scorm, $action, $cm) {
    ...
    // if ($scorm->displaycoursestructure == 1) {
        ...
        ?>
		<?php if($PAGE->bodyid == 'page-mod-scorm-view'){ ?>
		<div class="scorm-resource">
              	<form id="scormviewform" method="post" action="<?php echo $CFG->wwwroot ?>/mod/scorm/player.php">
              		<input class="enter_scorm" type="submit" value="<?php print_string('enter', 'scorm') ?>" />
              		<input type="hidden" name="scoid"/>
              		<input type="hidden" name="cm" value="<?php echo $cm->id ?>"/>
              		<input type="hidden" name="currentorg" value="<?php echo $orgidentifier ?>" />
              	</form>
        	...
        	<div class="structurehead" style="font-family:ProximaNovaSoft-Bold;"><br /><?php print_string('summary', 'scorm') ?></div>
		<?php } ?>
