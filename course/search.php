...
if (!empty($blocklist) and confirm_sesskey()) {
	...
	$scormsByCourse = get_scorms_search($searchterms, "fullname ASC", $page, $perpage, $totalcount);
}
...
if ($courses) {
    ...
    if (!$adminediting) { //Not editing mode
        foreach ($courses as $course) {
            $course->summary = "";
        	//Iterates the scorm matches to add them below each course in case they exist
       		foreach($scormsByCourse[$course->id] as $scorm){
				$course->summary .= "</br>Activities: <a href=\"../mod/scorm/view.php?id=$scorm->course_module_id\">".$scorm->scorm_name."</a>";
       		}
        		
            // front page don't belong to any category and block can exist.
            if ($course->category > 0) {
            	//Got rid of this because Jim´s order, it will be concatenated on the same line than the course            					
                ...
                $course->summary .= "</a></p>";				  
				 */				
				$course->category_text .= $displaylist[$course->category];				
            }
