            ...
            foreach ($courses as $course) {
                ...
                if ($course->summary) {
                    ...
                    $actionlink = $OUTPUT->action_link($link, '<img alt="'.$strsummary.'" src="'.$OUTPUT->pix_url('i/info') . '" />',
                        new popup_action('click', $link, 'courseinfo', array('height' => 400, 'width' => 900)),
...
function print_course($course, $highlightterms = '') {
    ...
    // Customized by Jim´s order
	echo "Course: ";
    echo html_writer::link($linkhref, $linktext, $linkparams);	
	// Added to concat the category on the same line than the course on the search results
	if(!empty($course->category_text)){
	 	$linkhrefCategory = new moodle_url('/course/category.php', array('id'=>$course->category));//
		$linkparamsCategory = array('title'=>'Click to enter the category');
	    echo "<span style='float:right;'>Category: ";
	    echo html_writer::link($linkhrefCategory, $course->category_text, $linkparamsCategory)."</span>";	
	}
    ...
    if (!empty($CFG->coursecontact)) {
		...
		/* Got rid of teacher on search results by Jim´s order
        if (!empty($namesarray)) {
