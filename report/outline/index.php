...
if($id>1){
	...
	$courseList = $DB->get_records_select('course', 'id>0', null, 'id', 'id', 0, 0);
	
	$courseList = array_values($courseList);
	
	//Variable to validate some of the lines that should only be set once
	$isFirstValue = true;
	
	foreach($courseList as $cid=>$cvalue){
		...
		if($isFirstValue){
			$PAGE->set_title('All Courses : '. $stractivityreport);
			$PAGE->set_heading('All Courses Activity Report');
			echo $OUTPUT->header();	
		}
